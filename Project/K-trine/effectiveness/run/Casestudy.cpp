...
int main(int argc, char **argv){
    ...
    while (1){
        ...
        int k;
        scanf("%d",&k);
        std::string out_graph_dir_ = basedir+"k"+std::to_string(k)+argv[1];
        FILE* ofile;
        if (argc == 3)
        ...
        for (int i = 0; i < n; ++i) {
            if (trine1.TrinessOut(i)>= (k - 1) * k)
                is_in_subG[i] ++;
            if (truss1.NodetrussOut(i)>= k + 1)
                is_in_subG[i] ++;
            if (core1.CorenessOut(i)>= k)
                is_in_subG[i] ++;
            if (is_in_subG[i]) {
                printf("%d %d\n", i, is_in_subG[i]);
                if (argc == 3)
        ...
        for (auto e : gedges) 
            if (is_in_subG[e.first] && is_in_subG[e.second]) {
                if (is_in_subG[e.first] == 2 || is_in_subG[e.second] == 2)
        ...
        if (argc == 3){
            std::string ind_graph_dir_ = basedir+"ind_k"+std::to_string(k)+argv[1];
            ...
            fprintf(indofile, "A\n");
            for (auto e : gedges) 
                if ((is_in_subG[e.first] == 2 || is_in_subG[e.second] == 2) && is_in_subG[e.first] && is_in_subG[e.second]) {
                    fprintf(indofile, "%d %d\n", e.first, e.second);
                }
            fclose(indofile);
            break;
