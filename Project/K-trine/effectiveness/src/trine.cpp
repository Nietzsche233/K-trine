...
namespace trine{
    ...
    double Trine::ClusteringCoefficient(uint32_t retio_) {
        uint32_t sub_n_ = 1ll * g_.n_ * retio_ / 100;
        if (retio_ >= 100) {
            uint32_t i;
            for (i = n_-1; i > 1; i--) {
                if(triness_[k_order[i-1]] < triness_[k_order[i]])
                    break;
            }
            sub_n_ = g_.n_ - i;
            printf("(%d-trine : %d)",triness_[k_order[i]],sub_n_);
        }
    ...
    double Trine::Density(uint32_t retio_) {
        uint32_t sub_n_ = 1ll * g_.n_ * retio_ / 100;
        if (retio_ >= 100) {
            uint32_t i;
            for (i = n_-1; i > 1; i--) {
